...
onload = () => {
    ...
    document.getElementById('info-user').innerHTML = `${contaAtual.user}`;
    document.getElementById('info-name').innerHTML = `${contaAtual.name}`;
    document.getElementById('info-email').innerHTML = `${contaAtual.email}`;
    document.getElementById('info-points').innerHTML = `${contaAtual.taskpoints}`;
...
document.querySelector('.recompensas').addEventListener('click', e => {
    e.preventDefault();

    location.href = '../pages/recompensas.html';
});